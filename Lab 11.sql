...
begin
    ...
    dbms_output.put_line('facluty_name: ' || IT.faculty_name);
    exception 
        when others -- конструкция секции исключений
...
begin
    update faculty 
        set faculty_name = 'TOV',
        faculty = 'Tekhnologiya organicheskih veshchestv'
        where faculty = 'TOV';
    --rollback;
    is_found := sql%found;
    is_open := sql%isopen;
...
declare
    ...
    rowcount pls_integer;
begin
    insert into faculty values ('Just some faculty', 'Just some faculty');
    --rollback;
    is_found := sql%found;
    is_open := sql%isopen;
    is_notfound := sql%notfound;
...
declare
    ...
    rowcount pls_integer;
begin
    delete faculty where faculty = 'TOV';
    --rollback;
    is_found := sql%found;
    is_open := sql%isopen;
    is_notfound := sql%notfound;
...
begin
    ...
    loop
        ...
        dbms_output.put_line(rpad(c_auditorium.auditorium, 21) ||
        rpad(c_auditorium.auditorium_type, 16) ||
        rpad(c_auditorium.auditorium_capacity, 20) ||
        rpad(c_auditorium.auditorium_name, 15));
    end loop;
